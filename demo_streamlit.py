...
def main():
    st.title("🤖 Memory Chatbot Demo")
    st.markdown("*Chatbot thông minh với bộ nhớ lâu dài sử dụng Google Gemini*")
    ...
    if not google_api_key:
        st.warning("⚠️ Vui lòng nhập Google API Key trong sidebar để bắt đầu chat")
        st.info("Bạn có thể lấy API key tại: https://makersuite.google.com/app/apikey")
        return
    
    # Khởi tạo chatbot
    try:
        if "chatbot" not in st.session_state or st.session_state.get("current_user_id") != user_id:
